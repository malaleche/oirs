...
if (isset($this->request->params['pass'][0])) {
    ...
    switch ($pass) {
        case 'Sin Asignar':$this->Html->addCrumb('Asignar', '#');
            break;
        case 'Solicitud Reasignacion':$this->Html->addCrumb('Reasignacion', '#');
            break;
        case 'Solicitud Tiempo Extendido':$this->Html->addCrumb('Extension', '#');
            break;
    }
} else {
    $this->Html->addCrumb('Anotaciones', '#');
}
?>
...
<!--			<th><?php // echo $this->Paginator->sort('cuerpo');    ?></th>-->
        ...
        <?php foreach ($anotaciones as $anotacion): ?>
           <?php 
            ...
            $fecha->add(new DateInterval('P'.$anotacion['Anotacion']['extension_plazo'].'D'));
